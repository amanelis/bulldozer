  ...
  for document in documents
    begin
      ...
      begin 
        ...
        page_text = document_page.css('.content_header_full').first
        # text = page_text.content.match(/(.*) for/)
        title = page_text.nil? ? "Document" : page_text.content
      ...
      document_obj = Document.create!({:university_id => university_obj.id, :professor_id => isStaff ? nil : professor_obj.id, :url => document_url, :path => path, :course_name => course_name, :style => type, :original_url => document_url, :title => title})

      professor_name = isStaff ? "STAFF" : professor_obj.last_name
